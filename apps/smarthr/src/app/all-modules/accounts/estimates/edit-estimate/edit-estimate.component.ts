...
export class EditEstimateComponent implements OnInit {
  ...
  savesend() {
    if (!this.editEstimateForm.valid) {
      ...
      const params = this.editEstimateForm.value;
      params.status = 0;
      params.id = 2;
      if (this.dateStatus) {
      ...
      const getItems = this.editEstimateForm.get('items').value;
      const amount = this.editEstimateForm.value.totalamount.toString();
      const obj = {
        number: 'EST-0001',
  ...
  edit(value) {
    ...
    const toSetValues = this.allEstimates[index];

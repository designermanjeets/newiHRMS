...
export class CreateEstimatesComponent implements OnInit {
  ...
  changePrice(i) {
    const qty = this.itemsList.at(i).get('qty').value;
    const price = this.itemsList.at(i).get('unit_cost').value;
  ...
  savesend() {
    if (!this.addEstimateForm.valid) {
      ...
      const estimateDateFormat = this.pipe.transform(
        this.addEstimateForm.value.estimate_date,
        'dd-MM-yyyy'
      );
      const expiryToDateFormat = this.pipe.transform(
        this.addEstimateForm.value.expiry_date,
        'dd-MM-yyyy'
      );
      const getItems = this.addEstimateForm.get('items').value;
      const amount = this.addEstimateForm.value.totalamount.toString();
      const obj = {
        number: 'EST-0001',

...
export class EditInvoiceReportComponent implements OnInit {
  ...
  savesend() {
    if (!this.editInvoiceForm.valid) {
      ...
      const params = this.editInvoiceForm.value;
      params.status = 0;
      params.id = 2;
      if (this.dateStatus) {
      ...
      const getItems = this.editInvoiceForm.get('items').value;
      const amount = this.editInvoiceForm.value.totalamount.toString();
      const obj = {
        number: '#INV-0001',
  ...
  edit(value) {
    ...
    const toSetValues = this.allInvoices[index];

...
export class CreateInvoiceComponent implements OnInit {
  ...
  savesend() {
    if (!this.addInvoiceForm.valid) {
      ...
      const invoiceDateFormat = this.pipe.transform(
        this.addInvoiceForm.value.invoice_date,
        'dd-MM-yyyy'
      );
      const dueDateFormat = this.pipe.transform(
        this.addInvoiceForm.value.due_date,
        'dd-MM-yyyy'
      );
      const getItems = this.addInvoiceForm.get('items').value;
      const amount = this.addInvoiceForm.value.totalamount.toString();
      const obj = {
        number: '#INV-0001',
